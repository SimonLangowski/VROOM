...
def better_colors(lineid):
    ...
    # Latency sweep labels (MontRNSReduceVecMont32stride=…)
    if "stride=1" in lineid:
        return colors[0]
    if "stride=2" in lineid:
        return colors[1]
    return colors[hash(lineid) % len(colors)]
...
def gpu_latency_plot(fn):
    ...
    outx, outy = better_titles(outx, outy)
    plot_things(outx, outy, "Modulus size (bits)", "Time (ns)", "latency", "Latency GPU comparison")
