...
INLINE auto bls12_381_batch_reduce_perm_bound(
    const Ring &ring,
    ...
    static_assert(Ring::TRUE_LIMBS == BLS12_381_TRUE_LIMBS && Ring::LIMBS == BLS12_381_PADDED_LIMBS,
        "BLS12-381 perm batch requires 8×8 ring");
    auto c_m2_acc = batch_change_base_perm<batch, Ring::QUO_EST_ERROR>(c_in, bls12_381_r1_perm::r1_fixed_perm_wide_reducer);
...
INLINE std::array<typename Ring::StandardElement, batch> bls12_381_batch_expand_scalar_r2(
    const Ring &ring,
    const std::array<typename Ring::SmallStandardElement, batch> &c_m2) {
    static_assert(Ring::TRUE_LIMBS == BLS12_381_TRUE_LIMBS && Ring::LIMBS == BLS12_381_PADDED_LIMBS,
        "BLS12-381 expand requires 8×8 ring");
...
template <class Ring, int batch>
INLINE std::array<typename Ring::StandardElement, batch> bls12_381_batch_reduce_expand_perm(
    const Ring &ring,
    const std::array<typename Ring::template ReadyElement<static_cast<int64_t>(Ring::MAX_ADD)>, batch> &elements) {
    static_assert(Ring::TRUE_LIMBS == BLS12_381_TRUE_LIMBS && Ring::LIMBS == BLS12_381_PADDED_LIMBS,
        "BLS12-381 reduce_expand requires 8×8 ring");
    auto cm2_acc = bls12_381_batch_reduce_perm_bound<Ring, batch>(ring, elements);
    ...
    return bls12_381_batch_expand_scalar_r2<Ring, batch>(ring, c_m2);
}
