...
static void BM_Inverse(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    BLS381AddChainInversion<BoundedRing<381, 8, 50>> inverter(ring);
...
static void BM_MillerLoop_DoubleStep(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    FP2Ring<BoundedRing<381, 8, 50>> fp2ring(ring);
    FP12Ring<BoundedRing<381, 8, 50>> fp12ring(ring);
    
    // Set up G1 and G2 points
    BigInt g1_x(g1_gen_x, 10);
    BigInt g1_y(g1_gen_y, 10);
    AffinePoint<typename BoundedRing<381, 8, 50>::StandardElement> E1_point{
    ...
    AffinePoint<FP2Ring<BoundedRing<381, 8, 50>>::StandardElement> E2_point{
        FP2Ring<BoundedRing<381, 8, 50>>::StandardElement(
    ...
    Miller<BoundedRing<381, 8, 50>> miller(E1_point, E2_point, ring);
    
    // Set up projective point and f for double step
    ProjectivePoint<FP2Ring<BoundedRing<381, 8, 50>>::StandardElement> P{
        E2_point.x,
        E2_point.y,
        FP2Ring<BoundedRing<381, 8, 50>>::one(ring)
    };
...
// Benchmark FP12 inversion using BLST-backed inverter for 50-bit ring
static void BM_FP12_Inverse_BLST(benchmark::State& state) {
    ...
    state.SetLabel("FP12_Inverse_BLST_50bit");
}

// Benchmark FP12 inversion using RNS flat inverse with BLST for 50-bit ring
static void BM_FP12_Inverse_RNS_BLST(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    FP12Ring fp12ring(ring);
    FP12RNS_BLSTInverter<BoundedRing<381, 8, 50>> inverter;
    ...
    state.SetLabel("FP12_Inverse_RNS_BLST_50bit");
}

// Benchmark final exponentiation using BLST-backed FP12 inverter for 50-bit ring
static void BM_FP12_FinalExp_BLST_Inverter(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    FP12Ring fp12ring(ring);
    FP12BLSTInverter<BoundedRing<381, 8, 50>> inverter;
    ...
    state.SetLabel("FP12_FinalExp_BLST_Inverter_50bit");
}

// Benchmark final exponentiation using RNS flat inverse with BLST for 50-bit ring
static void BM_FP12_FinalExp_RNS_BLST_Inverter(benchmark::State& state) {
    ...
    state.SetLabel("FP12_FinalExp_RNS_BLST_Inverter_50bit");
}

// Benchmark full pairing (Miller loop + final exponentiation) using BLST-backed FP12 inverter for 50-bit ring
static void BM_Pairing_BLST_Inverter(benchmark::State& state) {
    ...
    state.SetLabel("Pairing_BLST_Inverter_50bit");
}

// Benchmark full pairing using RNS flat inverse with BLST for 50-bit ring
static void BM_Pairing_RNS_BLST_Inverter(benchmark::State& state) {
    ...
    state.SetLabel("Pairing_RNS_BLST_Inverter_50bit");
}
...
static void BM_BatchReduceExpand_1(benchmark::State& state) {
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 1> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 1> b_elements;
    for (int i = 0; i < 1; i++) {
    ...
    std::array<typename BoundedRing<381, 8, 50>::ReadyElement<BoundedRing<381, 8, 50>::MAX_ADD>, 1> ready_elements;
    for (int i = 0; i < 1; i++) {
        auto product = a_elements[i] * b_elements[i];
        ready_elements[i] = ring.template ready<BoundedRing<381, 8, 50>::MAX_ADD>(product);
    }
...
static void BM_BatchReduceExpand_2(benchmark::State& state) {
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 2> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 2> b_elements;
    for (int i = 0; i < 2; i++) {
        BigInt val_a(rng.get_z_range(p.get_mpz()));
        BigInt val_b(rng.get_z_range(p.get_mpz()));
        a_elements[i] = ring.from_bigint(val_a);
    ...
    // Create products and convert to ReadyElement array
    std::array<typename BoundedRing<381, 8, 50>::ReadyElement<BoundedRing<381, 8, 50>::MAX_ADD>, 2> ready_elements;
    for (int i = 0; i < 2; i++) {
        auto product = a_elements[i] * b_elements[i];
        ready_elements[i] = ring.template ready<BoundedRing<381, 8, 50>::MAX_ADD>(product);
    }
...
static void BM_BatchReduceExpand_3(benchmark::State& state) {
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 3> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 3> b_elements;
    for (int i = 0; i < 3; i++) {
        BigInt val_a(rng.get_z_range(p.get_mpz()));
        BigInt val_b(rng.get_z_range(p.get_mpz()));
        a_elements[i] = ring.from_bigint(val_a);
    ...
    // Create products and convert to ReadyElement array
    std::array<typename BoundedRing<381, 8, 50>::ReadyElement<BoundedRing<381, 8, 50>::MAX_ADD>, 3> ready_elements;
    for (int i = 0; i < 3; i++) {
        auto product = a_elements[i] * b_elements[i];
        ready_elements[i] = ring.template ready<BoundedRing<381, 8, 50>::MAX_ADD>(product);
    }
...
static void BM_BatchReduceExpand_4(benchmark::State& state) {
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 4> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 4> b_elements;
    for (int i = 0; i < 4; i++) {
        BigInt val_a(rng.get_z_range(p.get_mpz()));
        BigInt val_b(rng.get_z_range(p.get_mpz()));
        a_elements[i] = ring.from_bigint(val_a);
    ...
    // Create products and convert to ReadyElement array
    std::array<typename BoundedRing<381, 8, 50>::ReadyElement<BoundedRing<381, 8, 50>::MAX_ADD>, 4> ready_elements;
    for (int i = 0; i < 4; i++) {
        auto product = a_elements[i] * b_elements[i];
        ready_elements[i] = ring.template ready<BoundedRing<381, 8, 50>::MAX_ADD>(product);
    }
...
static void BM_BatchReduceExpand_5(benchmark::State& state) {
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 5> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 5> b_elements;
    for (int i = 0; i < 5; i++) {
        BigInt val_a(rng.get_z_range(p.get_mpz()));
        BigInt val_b(rng.get_z_range(p.get_mpz()));
        a_elements[i] = ring.from_bigint(val_a);
    ...
    // Create products and convert to ReadyElement array
    std::array<typename BoundedRing<381, 8, 50>::ReadyElement<BoundedRing<381, 8, 50>::MAX_ADD>, 5> ready_elements;
    for (int i = 0; i < 5; i++) {
        auto product = a_elements[i] * b_elements[i];
        ready_elements[i] = ring.template ready<BoundedRing<381, 8, 50>::MAX_ADD>(product);
    }
...
static void BM_BatchReduceExpand_6(benchmark::State& state) {
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 6> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 6> b_elements;
    for (int i = 0; i < 6; i++) {
        BigInt val_a(rng.get_z_range(p.get_mpz()));
        BigInt val_b(rng.get_z_range(p.get_mpz()));
        a_elements[i] = ring.from_bigint(val_a);
    ...
    // Create products and convert to ReadyElement array
    std::array<typename BoundedRing<381, 8, 50>::ReadyElement<BoundedRing<381, 8, 50>::MAX_ADD>, 6> ready_elements;
    for (int i = 0; i < 6; i++) {
        auto product = a_elements[i] * b_elements[i];
        ready_elements[i] = ring.template ready<BoundedRing<381, 8, 50>::MAX_ADD>(product);
    }
...
static void BM_BatchReduceExpand_12(benchmark::State& state) {
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 12> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 12> b_elements;
    for (int i = 0; i < 12; i++) {
        BigInt val_a(rng.get_z_range(p.get_mpz()));
        BigInt val_b(rng.get_z_range(p.get_mpz()));
        a_elements[i] = ring.from_bigint(val_a);
    ...
    // Create products and convert to ReadyElement array
    std::array<typename BoundedRing<381, 8, 50>::ReadyElement<BoundedRing<381, 8, 50>::MAX_ADD>, 12> ready_elements;
    for (int i = 0; i < 12; i++) {
        auto product = a_elements[i] * b_elements[i];
        ready_elements[i] = ring.template ready<BoundedRing<381, 8, 50>::MAX_ADD>(product);
    }
...
// Benchmark FP2 multiplication using FP2Ring for 50-bit ring
static void BM_FP2_Mul(benchmark::State& state) {
    ...
    auto a = FP2Ring<BoundedRing<381, 8, 50>>::StandardElement(
        ring.from_bigint(ax),
        ring.from_bigint(ay)
    );
    auto b = FP2Ring<BoundedRing<381, 8, 50>>::StandardElement(
        ring.from_bigint(bx),
        ring.from_bigint(by)
    );
    ...
    state.SetLabel("FP2_Mul_50bit");
}

// Benchmark FP2 squaring using FP2Ring for 50-bit ring
...
static void BM_BatchModMul_1(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<1>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_1");
}

static void BM_BatchModMul_2(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<2>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_2");
}

static void BM_BatchModMul_3(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<3>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_3");
}

static void BM_BatchModMul_4(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<4>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_4");
}

static void BM_BatchModMul_5(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<5>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_5");
}

static void BM_BatchModMul_6(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<6>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_6");
}

static void BM_BatchModMul_7(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 7> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 7> b_elements;
    for (int i = 0; i < 7; i++) {
        BigInt val_a(rng.get_z_range(p.get_mpz()));
        BigInt val_b(rng.get_z_range(p.get_mpz()));
        a_elements[i] = ring.from_bigint(val_a);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<7>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_7");
}

static void BM_BatchModMul_8(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 8> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 8> b_elements;
    for (int i = 0; i < 8; i++) {
        BigInt val_a(rng.get_z_range(p.get_mpz()));
        BigInt val_b(rng.get_z_range(p.get_mpz()));
        a_elements[i] = ring.from_bigint(val_a);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<8>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_8");
}

static void BM_BatchModMul_10(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 10> a_elements;
    std::array<typename BoundedRing<381, 8, 50>::StandardElement, 10> b_elements;
    for (int i = 0; i < 10; i++) {
        BigInt val_a(rng.get_z_range(p.get_mpz()));
        BigInt val_b(rng.get_z_range(p.get_mpz()));
        a_elements[i] = ring.from_bigint(val_a);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<10>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_10");
}

static void BM_BatchModMul_12(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50> ring(p);
    ...
    for (auto _ : state) {
        auto result = ring.batch_modmul<12>(a_elements, b_elements);
        benchmark::DoNotOptimize(result);
    }
    
    state.SetLabel("BatchModMul_12");
}
...
static void BM_G1_Add(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50, -1932, 2377, 12> ring(p);
    G1<decltype(ring)> g1;
...
// Benchmark GLV scalar multiplication on G1 (byte array version, 50-bit configuration)
static void BM_G1_ScalarMult_GLV_50bit(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50, -1932, 2377, 12> ring(p);
    using RingType = decltype(ring);
    ...
    state.SetLabel("G1_ScalarMult_GLV_50bit");
}

// Benchmark GLS scalar multiplication on G2 (byte array version, 50-bit configuration)
static void BM_G2_ScalarMult_GLS_50bit(benchmark::State& state) {
    BigInt p = BigInt(modulus, 10);
    BoundedRing<381, 8, 50, -1932, 2377, 12> ring(p);
    using RingType = decltype(ring);
    ...
    state.SetLabel("G2_ScalarMult_GLS_50bit");
}

BENCHMARK(BM_ModMul);
BENCHMARK(BM_BatchModMul_1);
BENCHMARK(BM_BatchModMul_2);
BENCHMARK(BM_BatchModMul_3);
BENCHMARK(BM_BatchModMul_4);
BENCHMARK(BM_BatchModMul_5);
BENCHMARK(BM_BatchModMul_6);
BENCHMARK(BM_BatchModMul_7);
BENCHMARK(BM_BatchModMul_8);
BENCHMARK(BM_BatchModMul_10);
BENCHMARK(BM_BatchModMul_12);
...
BENCHMARK(BM_G1_ScalarMult_GLV_50bit);
BENCHMARK(BM_G2_ScalarMult_GLS_50bit);
