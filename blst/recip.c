...
#else
/* sqr_n_mul_fp: square n times then multiply by b */
...
/* flt_reciprocal_fp2 removed - only used in signing operations, not needed for pairing/scalar_mul */
...
/*
 * Wrapper for ct_inverse_mod_384 that works with normal (non-Montgomery) form.
 * 
 * Input:  inp in normal form (a)
 * Output: out in normal form (a^-1 mod p)
 * 
 * Note: ct_inverse_mod_384 uses extended Euclidean algorithm which is
 * representation-agnostic, so we can call it directly with normal form input.
 * The output is a 768-bit value in normal form. We use redc_mont_384 for
 * efficient 768-to-384-bit reduction (it's the fastest way even though it
 * does Montgomery conversion), then convert back with from_mont_384.
 */
void ct_inverse_mod_384_wrapper(vec384 out, const vec384 inp)
{
    ...
    /* Compute inverse directly with normal form input: outputs a^-1 (768-bit, normal form) */
    ct_inverse_mod_384(temp.x, inp, BLS12_381_P, Px8);
    
    /* Reduce 768-bit to 384-bit using Montgomery reduction, then convert back to normal form.
     * redc_mont_384 outputs in Montgomery form, so we use from_mont_384 to convert back. */
    redc_mont_384(temp.r[0], temp.x, BLS12_381_P, p0);
