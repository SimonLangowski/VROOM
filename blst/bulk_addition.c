...
\
    ...
    if (mul_acc != NULL) { \
        vec##bits tmp; \
        vec_copy(tmp, *mul_acc, sizeof(tmp)); \
        mul_##field(A->Z, A->Z, tmp);	/* chain multiplication */ \
    } \
}
...
void ptype##s_accumulate(ptype *sum, ptype points[], size_t n) \
{ \
    ptype *dst; \
    const vec##bits *mul_acc; \
    size_t i; \
\
    while (n >= 16) { \
        if (n & 1) \
            ptype##_dadd_affine_public(sum, sum, (const ptype##_affine *)points++); \
        n /= 2; \
        for (mul_acc = NULL, i = n; i--; mul_acc = (const vec##bits *)points->Z, points += 2) \
            ptype##_head(points, mul_acc); \
\
        if (sizeof(points[-2].Z) == sizeof(vec384)) \
            blst_fp_inverse((limb_t*)points[-2].Z, (const limb_t*)points[-2].Z); \
        else { \
            vec384x tmp_z; \
            vec_copy(tmp_z, points[-2].Z, sizeof(tmp_z)); \
            blst_fp2_inverse(tmp_z, tmp_z); \
            vec_copy(points[-2].Z, tmp_z, sizeof(tmp_z)); \
        } /* 1/∏ Zi */ \
\
...
{ \
    ...
    ptype *scratch = (ptype *)alloca((npoints > stride ? stride : npoints) * \
                            sizeof(ptype)); \
