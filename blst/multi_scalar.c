...
\
        ...
        ptype##_affine *row_ptr = (ptype##_affine *)(&table[top]); \
        ptype *rows_ptr = (ptype *)(&table[top]); \
        rows = rows_ptr; \
        row = row_ptr; \
        for (i = 0; i < stride; i++, row += nwin) \
            point = *points ? *points++ : point+1, \
            ptype##_precompute_row_wbits((ptype *)row, wbits, point); \
        ptype##s_to_affine_row_wbits(row_ptr, rows_ptr, wbits, stride); \
        top += stride << (wbits-1); \
        npoints -= stride; \
    } \
    ptype##_affine *rows_local = (ptype##_affine *)alloca(2*sizeof(ptype##_affine) * npoints * nwin); \
    row = rows_local; \
    rows = (ptype *)(void *)rows_local; \
    for (i = 0; i < npoints; i++, row += nwin) \
        point = *points ? *points++ : point+1, \
        ptype##_precompute_row_wbits((ptype *)row, wbits, point); \
    ptype##s_to_affine_row_wbits(rows_local, rows, wbits, npoints); \
} \
\
...
\
    ...
    if (scratch == NULL) { \
        ...
        scratch = (ptype *)alloca(sizeof(ptype) * scratch_sz); \
    } \
\
...
{ \
    ...
    if ((npoints * sizeof(ptype##_affine) * 8 * 3) <= SCRATCH_LIMIT && \
        npoints < 32) { \
        ptype##_affine *table = (ptype##_affine *)alloca(npoints * sizeof(ptype##_affine) * 8); \
