                                        ...
                                        const POINTonE1_affine Px2[], size_t n)
{
    ...
    vec_zero(ret, sizeof(vec384fp12));
    vec_copy(ret[0][0], line[0], 2*sizeof(vec384fp2)); // xy0 000
    // ret[2:w][3:v][2:u]
    vec_copy(ret[1][1], line[2], sizeof(vec384fp2));   // xy0 0z0
                                          ...
                                          const POINTonE1_affine P[], size_t n)
{
    ...
    POINTonE2 *T = (POINTonE2 *)alloca(n*sizeof(POINTonE2));
    POINTonE1_affine *Px2 = (POINTonE1_affine *)alloca(n*sizeof(POINTonE1_affine));
#else
    POINTonE2 T[n];
...
void blst_line_add(void *line, void *T, const void *R, const void *Q)
{   line_add(*(vec384fp6 *)line, (POINTonE2 *)T, (const POINTonE2 *)R,
             (const POINTonE2_affine *)Q);   }

void blst_line_dbl(void *line, void *T, const void *Q)
{   line_dbl(*(vec384fp6 *)line, (POINTonE2 *)T, (const POINTonE2 *)Q);   }

void blst_line_by_Px2(void *line, const void *Px2)
{   line_by_Px2(*(vec384fp6 *)line, (const POINTonE1_affine *)Px2);   }
